...
controller.showStartPage = (req, res) => {
...
controller.showSignUpWithEmail = (req, res) => {
...
controller.registerEmail = (req, res) => {
  ...
  const resetLink = `${req.protocol}://${host}/register?token=${sign(
    email
  ...
  sendRegisterMail(userName, email, host, resetLink)
    ...
    .catch((error) => {
      ...
      return res.render("signupEmail", {
        message: "Error occur , Failed to send email!, check your mail address",
      });
    });
};
...
controller.forgotPassword = async (req, res) => {
  ...
  if (user) {
    ...
    const { sign } = require("./jwt");
    const host = req.header("host");
    const resetLink = `${req.protocol}://${host}/reset?token=${sign(
      email
    )}&email=${email}`;
    // console.log("resetlink" + resetLink);
    // Send email
    ...
    sendForgotPasswordMail(user, host, resetLink)
      ...
      .catch((error) => {
        ...
        return res.render("forgotPassword", {
          message:
            "Error occurred, Failed to send email!, check your mail address",
        });
      });
  } else {
    // Otherwise, notify that the email does not exist
